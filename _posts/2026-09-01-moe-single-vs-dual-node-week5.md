...
하드웨어는 그대로인데 어떤 작업을 시키느냐에 따라 결론이 뒤집힌다. 이유를 살펴보자.
...
하드웨어는 그대로인데 작업 성격만 바뀌었더니 결과가 4.3배에서 1.02배로 변했다. 그렇다면 차이를 만든 건 하드웨어 자체가 아니라, 그 작업이 하드웨어의 어느 부분을 쓰느냐다.

## 4.3배 중 얼마가 노드를 나눈 대가인가

4.3배를 전부 노드를 나눈 대가로 볼 수는 없다. NVFP4는 FP8보다 가중치를 절반만 읽으면 되니, 노드 수와 관계없이 그 자체로 빠르다. 이 몫부터 따로 떼어놓고 봐야 한다.

디코드 구간이 기준이 된다. 디코드는 메모리에서 가중치를 읽어오는 속도가 성능을 좌우하고 노드 간 통신량은 적다. 그러니 여기서 나는 차이는 대부분 정밀도 때문이라고 봐도 된다.
...
정밀도가 설명하는 몫을 2배로 놓으면, 프리필의 3.3~4.5배 중 남는 것이 **약 2배, 노드를 나눈 대가**다.
...
이유는 간단하다. 프리필은 한 번에 수천 개의 토큰을 전문가들에게 나눠 보내지만, 디코드는 요청당 한 토큰만 보낸다. 그러니 오가는 트래픽 양도 그 순간 처리 중인 토큰 수를 그대로 따라갈 수밖에 없다.
...
참고로 이번 두 노드 구성은 rail-optimized 설계였다. 그래서 노드 간 트래픽이 스파인까지 올라가지 않고 리프 스위치 한 홉으로 끝났다. 두 노드로 나눌 때 경로상 가장 유리한 조건인 셈이라, 랙을 넘어가는 실제 환경이라면 이보다 나아지기는 어렵다.

결국 한 노드 구성이 앞선 것은 4비트로 압축해서만이 아니다. **압축한 덕분에 EP 전체가 NVLink 안에 들어왔기 때문**이다. 이 비교가 실제로 잰 것은 양자화와 분산의 우열이 아니라, 전문가 분산을 한 노드 안에 가둬뒀을 때 얻는 이득이다.
...
처리량 숫자만 보고 튜닝하면 이 지점을 놓치기 쉽다. 처리량은 완만하게 떨어지는 반면 지연 시간은 순식간에 자릿수가 달라지기 때문이다.
...
옵션을 켰다고 실제로 동작했다는 보장은 없다. 이 효과를 보려면 같은 시스템 프롬프트가 반복되는 데이터를 따로 만들어야 하는데, 이번 테스트에는 그런 데이터가 없었다. 그러니 "캐시 재사용을 켜고 쟀다"는 말 자체가 큰 의미는 없는 셈이다.
