...
답부터 말하면 **거의 전적으로 추론 쪽**이다. 처음엔 나도 "둘 다 쓰겠지" 싶었는데, 이유를 따져보니 명확했다. 캐시가 쓸모 있으려면 두 가지 조건이 필요하다.
...
미리 밝혀두면, 이 글의 벤더 성능 수치는 전부 벤더가 자기 환경에서 측정해 발표한 값이다. 내가 재현한 게 아니다. 대신 계산으로 검증할 수 있는 부분(캐시 용량, 전송 시간)은 직접 손으로 계산해서 넣었고, 어디까지가 내 계산이고 어디부터가 인용인지 구분해 뒀다.
...
1주차에 microGPT를 보면서 "KV Cache라는 게 결국 익숙한 캐싱 개념이구나" 하고 안심했는데, 2주차에 파고들어 보니 그 안심이 좀 성급했다. 개념은 익숙한 게 맞지만, 그 익숙한 개념이 42GB라는 크기를 만나는 순간 완전히 다른 문제가 된다는 걸 이번에 알았다.

그동안 "AI 인프라"라고 하면 GPU 성능을 먼저 떠올렸는데, 추론 시장이 커질수록 **GPU가 모자란 부분을 스토리지가 어떻게 메울 것인가**가 화두였다. 스토리지 회사들이 추론 시장에 뛰어들고 NVIDIA가 새 계층까지 만드는 이유가 여기 있었다.

솔직하게 남겨둘 것도 몇 가지 있다.

첫째, **1주차에 숙제로 남겼던 Adam의 bias correction은 이번에도 못 봤다.** KV Cache 쪽을 파느라 시간이 다 갔다. 다음에 학습 쪽을 다시 볼 일이 생기면 그때 정리하겠다.

둘째, **4장의 손익분기 계산은 어디까지나 어림값이다.** 특히 연산 성능을 실제로 40%만 쓴다고 잡은 부분은 내가 임의로 정한 값이라 실제 환경에서 크게 달라질 수 있다. 자릿수 감각을 잡는 용도로만 봐야 한다.

셋째, **CMX/G3.5가 실제로 얼마나 큰 변화일지는 아직 판단이 안 선다.** 스토리지 계층을 하나 더 만드는 게 근본적인 해법인지, 아니면 현재 병목에 대한 과도기적 대응인지 잘 모르겠다. 벤더 자료마다 자기에게 유리한 방향으로 서술하고 있어서 더 헷갈린다. 2026년 하반기에 실제 물건이 나오고 독립적인 벤치마크가 나와야 판단할 수 있을 것 같다.

넷째, **직접 돌려본 게 없다.** 1주차에는 코드를 로컬에서 실행해보고 손실값까지 재현했는데, 이번 주는 논문과 벤더 문서를 읽고 계산기를 두드린 게 전부다. 이 부분이 좀 아쉽다.

다만 완전히 맨땅은 아니었다. 얼마 전에 prefill부터 GPU 세대까지 추론 성능의 변천사를 16개 챕터로 정리해본 적이 있는데, 그때 훑어둔 것들이 이번에 크게 도움이 됐다. Orca의 continuous batching, PagedAttention, FlashAttention, GQA, Sarathi의 chunked prefill, Splitwise의 단계 분리 — 그때는 "시기별로 나온 기법들의 목록"처럼 보였다.

그런데 이번에 KV Cache라는 축 하나를 잡고 다시 보니 전혀 다르게 읽혔다. 저 기법들이 전부 같은 문제를 서로 다른 층위에서 공격하고 있었다. GQA는 캐시를 **작게** 만들고, PagedAttention은 **알뜰하게** 담고, prefix caching은 **나눠 쓰고**, chunked prefill은 **읽는 순서를 바꾸고**, disaggregation과 offloading은 **다른 곳에 옮긴다**. 목록이 아니라 계보였던 것이다.

같은 자료를 두 번 봤는데 두 번째에 전혀 다른 게 보인 경험이라 기록해둔다. 아마 첫 번째에는 "무엇이 있는가"를 봤고 이번에는 "왜 그게 필요했는가"를 봤기 때문인 것 같다.

**다음 주에는 손을 움직여볼 계획이다.** vLLM에 prefix caching을 켜고 끄면서 TTFT가 실제로 얼마나 달라지는지 측정해보고 싶다. 이번 글이 계산으로만 끝난 부분을 숫자로 확인해보는 게 목표다.
